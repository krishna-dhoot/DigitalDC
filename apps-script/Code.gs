  ...
  const payload = {
    ...
    messages: [{
      ...
      content: [
        ...
        { type: 'text', text: 'Extract this construction delivery challan into the delivery_challan tool, exactly as handwritten -- do not standardize or guess a "proper" name for the vendor or any material, just transcribe what is written. It is a printed pad with Marathi field labels. This chit design always follows this fixed order top to bottom, and the printed labels next to each line confirm it, but use BOTH together (the position, not only the label text) when reading each line, since small printed Marathi labels can themselves be hard to make out in a phone photo: DC number is top-left, date is top-right, then down the page -- (1) मालाचे नांव / material name, (2) माप / quantity (with unit alongside it, see the qty/unit field descriptions), (3) सप्लायर्सचे नांव / supplier-vendor name, (4) वाहन क्रमांक / vehicle number (in a boxed area), (5) साईट पत्ता / site name-address -- with a signature area at the very bottom that is not a data field. Flag anything illegible or ambiguous with low confidence rather than guessing silently.' + listsPrompt },
      ],
    }],
  };
